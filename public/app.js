...
async function loadRandomVideos(type) {
  if (!type) return;
  ...
  try {
    const response = await fetch(`/api/random?type=${encodeURIComponent(type)}`);
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Random fetch failed');
    }

    const data = await response.json();
    const videos = (Array.isArray(data.results) ? data.results : []).map((video, index) => {
      const safeVideo = video && typeof video === 'object' ? { ...video } : {};
      const fallbackId = `${type}-random-${Date.now()}-${index}`;
      const fallbackUrl = safeVideo.url || safeVideo.embedUrl || '';
      safeVideo.id = safeVideo.id || fallbackId;
      safeVideo.url = safeVideo.url || fallbackUrl;
      safeVideo.embedUrl = safeVideo.embedUrl || fallbackUrl;
      return safeVideo;
    });
    currentVideos = videos;
    currentPage = 1;
    totalPages = Math.ceil(videos.length / VIDEOS_PER_PAGE);
    displayResults(videos, `${type}-random`);
    ...
    resultsDiv.innerHTML = `<div class="error">ランダム取得エラー: ${error.message}</div>`;
    sortContainer.classList.add('hidden');
  } finally {
    loadingDiv.classList.add('hidden');
  }
...
function extractOriginalUrlFromProxyUrl(proxyUrl) {
...
function ensureRandomSortOption() {
  if (!sortSelect) return;
  const hasRandom = Array.from(sortSelect.options).some((option) => option.value === 'random');
  if (hasRandom) return;
  const option = document.createElement('option');
  option.value = 'random';
  option.textContent = 'ランダム';
  const titleAscOption = Array.from(sortSelect.options).find((item) => item.value === 'title-asc');
  if (titleAscOption) {
    sortSelect.insertBefore(option, titleAscOption);
  } else {
    sortSelect.appendChild(option);
  }
}

ensureRandomSortOption();
...
function isIPhone() {
  ...
  return (/iPhone|iPod|iPad/.test(ua) || iPadOS) && !window.MSStream;
}
...
document.querySelectorAll('.random-btn').forEach(btn => {
  const type = btn.getAttribute('data-random-type');
  if (type) {
    btn.addEventListener('click', () => loadRandomVideos(type));
  }
});
